...
`/prs` lists the open pull requests you authored across your installations, one row each: the arm circle (the arm and disarm button; filled means armed, and arming queues a review for the current head and marks the PR for review on every later push), the title with repo and number, one status word (Armed, Queued, Waiting with the runner offline, Reviewing, Run failed, or the last verdict), the round count, open findings and when the last review ended. The header shows how many are armed and reviewing and whether a runner is online, with an offline sentence linking to `/connect` when jobs are waiting. After upgrading, sign out and in once so your GitHub login is stored. Every armed row links to a per-PR page listing the PR's runs, with status, verdict (and the model's own verdict when it disagreed), turns, duration and the posted review link, and its findings with their stable ids, the head each was first seen on and whether it was resolved. The app resolves `@hawkeye/core` through the workspace link to its built `dist`, so `pnpm --filter @hawkeye/core build` (or `pnpm build`) has to run before `next dev` or `next build`; `pnpm --filter web typecheck` checks against core's sources and needs no build.
