...
**Per-pull-request pages** list the PR's runs with status, verdict (and the model's own verdict when it disagreed), turns, duration and the posted review link, and its findings with their stable ids, the head each was first seen on and whether it was resolved. When the latest run reviewed but its review went out in the short form or did not reach GitHub, a state sentence above the verdict says so, with GitHub's reason, so the dashboard explains a short or missing comment. While reviews are on and a review has been posted, a Review from scratch button beside the review control queues a job that reviews the full merge-base diff with no prior findings and no interdiff (`job.from_scratch`); its round replaces the living review in place as round 1 of a new history, later rounds count from it, the rounds before it drop out of view, and their inline threads are left as they are.
