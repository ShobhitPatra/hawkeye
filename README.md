...
- **The sandbox.** `npx hawkeye-review runner --sandbox docker` (or `podman`) runs each review's CLI inside a container that sees only the run directory (the checkout and the prompt) and the CLI's own login files (`~/.claude`, `~/.claude.json`, `~/.codex`), with your user id and the host network. The rest of your home directory and your environment are out of reach. The first start builds a small image (Node, git, the Claude Code and Codex CLIs) once. This is new and not yet proven against a hostile pull request; treat it as a wall under construction, not a finished one.
