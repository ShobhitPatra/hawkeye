...
Confirming an action is a state word changing, with at most a 160ms color transition; a refused field carries its sentence beneath it (`hk-refusal` on `hk-help`, `aria-invalid` on the input) and the save row says Not saved. Content that arrives, on first paint, on a refresh or after a click, fades in over 200ms with a 3px rise (`.hk-arrive`, keyed so a changed word is a new element); nothing exits with motion. A pressed button scales to 0.98 for 120ms; hover styles apply only to a fine pointer. Reduced motion turns everything static. No pulsing dots, no spinners, no scroll reveals, no animated numbers.
...
One bar, 44px, with a hairline beneath it, on every page. Signed in: the mark alone, linking to Overview (the word is hidden text so the link keeps its name; signed in, the name would only repeat the nav), then Overview, Pull requests, Runners, Settings, then at the right the small Updated 12 s ago line, an amber runner status only when a runner has waiting jobs, and the account name. The name opens a menu (`hk-menu`) holding the theme choice, the installation link and sign out, so Settings stays about review behavior. No avatar. The bar is sticky on dashboard pages and scrolls away on the landing. The current page is the foreground color; nothing is underlined. Under 760px the four words stay inline and never wrap, the nav scrolls sideways if it must, the name collapses to its initial, the status word hides, and the menu panel anchors to the bar's right edge instead of its summary; under 420px the signed-out wordmark keeps the mark alone too (`hk-wordmark-text` is clipped, so the link keeps its name). Both menus close on an outside click, on Escape and on navigation. Signed out: the wordmark, Source and a Sign in button; the theme choice lives in the footer. The same bar on a site page for someone already signed in (the prose pages and the not-found page) swaps that button for a plain Dashboard button in the same place, linking to Overview, so the bar never offers a sign-in that is not needed.
...
The root route for a signed-out visitor. It is for someone who just read a Hawkeye comment on a pull request, then for someone who followed a link. It must prove the reviews are worth reading before it says what they cost. The headline block comes first: the headline naming both harnesses in one line, which is the moat and stays so, the lede saying what the review does (reads the whole repository, not just the diff, under the user's own login, holds each change to the repository's own rules, and posts one verdict with what breaks, where and how to fix it), the sign-in button at its large size (`hk-button[data-size="large"]`, the one place that size is used) and the `npx hawkeye-review prepare <pr-url>` command in a code block with the copy icon as the no-account path. Then the hero: a flow line naming three acts (Open a pull request, The review runs on your machine, The review is posted), the current act in the foreground colour, done acts in secondary, and the connector between two acts filling left to right over the act's length, above one fixed-height stage (560px) that replays a real review of Hawkeye's own pull request, never a constructed example, screenshot or video. The stage shows one act at a time: the pull request as GitHub lists it with the pending Hawkeye status, then your machine's terminal with the daemon's own claimed, reviewing and posted lines, then the comment as GitHub renders it, first the shimmering Reviewing badge alone, then the review streaming in verdict first, the panel scrolling with it and settling at the top. The stage never grows; the comment scrolls inside it. A Replay control sits in the stage's bar. The review is a fixture rendered by the shipped renderer, and the caption links to the real pull request. Reduced motion shows the finished comment, every act lit. The landing owns one motion exception beyond the system's: the replay. Then five sections, each headed by what the visitor wants to know: what a review gives you (four claims on the steps row without numbers: the verdict follows the findings, every finding says what breaks and what to do, pinned to the changed line when there is one, the repository's own rules, the follow-up on every push), what it caught on its own pull requests (three findings from Hawkeye's reviews of itself, quoted word for word, never invented, in the margin column exactly as the dashboard renders findings, each gutter linking to the comment where it was raised), what happens when you open a pull request (four numbered steps, each carrying what does not leave the machine, with the cost as one quiet line beneath: the plan's limits are the budget and the dashboard shows what each review took), how to start, and "Open source, and reviewed by itself" (three facts on the steps row without numbers: the license and the self-review linking the repository's pull requests, the hosted instance in preview, how to contribute; self-hosting is documented but never linked from the landing). Proof is the three quoted findings, the repository link, the link to the real pull request, and the public reviews on the repository's pull requests. Primary action is "Sign in with GitHub". The footer is one line at the bottom of the page: the lockup, then Open source MIT, Reviews, Security, Privacy, Terms and the npm package, and at the right edge the theme choice as three words, Light, Dark and System, the current one in the foreground colour (`hk-footer-theme`, a radio group with a hidden legend), nothing else. Under two screens on a laptop. Same rules as every other surface, plus the headline size. Signing in lands on Overview, which is the dashboard home; a signed-in visit to the root route redirects there, and the wordmark in the top bar links there.
...
Settings are a column of sections, each a heading, one sentence, and the controls, at `/settings`, the fourth word in the top bar. Sections exist only for settings the product acts on. Reviews is the first: two checkboxes stacked (`hk-choice[data-stack]`), "Review my pull requests automatically" and "Review drafts too", then the field "Wait after a push" as a whole number with the unit word beside it and a help line under it. Runner is the second, under a rule: its sentence says reviews run with Claude Code or Codex under the user's own login, then Model as one choice in two groups side by side, one per CLI (Claude Code, Codex), each group a stacked `hk-choice` whose legend reads as a label. Each group lists its pinned models (no floating "latest" aliases) with a hint in secondary text after a middle dot, the same three words in both groups so the rows line up: most capable, balanced, fastest (Fable 5.1, Opus 5.5, Sonnet 5; GPT-6-Astra, GPT-6-Sol, GPT-6-Luna), then that CLI's default. Picking a model picks the CLI; nothing shows or hides. A saved model that has left the list stays as its own checked option at the end of its group, marked "no longer offered", until another is picked; a saved model that belongs to the other CLI counts as the default. The groups sit side by side while both fit, so the rows line up across the two CLIs, and stack 16px apart when they do not; no breakpoint forces it. One help line under the choice says the runner reviews with that CLI signed in on the user's machine, that a runner without it fails the review and says so, and that a runner started with --model keeps its own model for Claude Code (a new model joins the list with a release; the page does not say so), then "Turns per review" (1 to 200), "Minutes per review" (1 to 60) and "Reviews at once" (1 to 3) as whole numbers, each with a help line naming the result it causes; the last says each review spends the plan, so more at once reaches its limits sooner. Theme stays in the account menu. When it exists, the contract override is a mono textarea with a sentence that it is appended to the review prompt on every run, and a link to the repo-level file. Save is a primary button per section in an action row; the sentence beside it states the result ("Saved. Applies from the next pull request event.") or the refusal in one sentence, no toast.
...
Everything else the CLI says is a sentence. `prepare` prints the round on one line, then `prompt`, `checkout` and `result` as labeled paths in a gutter, then the next command to run. Connecting, the first `runner` on a machine or `runner login`, prints where to type the code, the code alone on its line as the one bold thing, the approval link, and that it is waiting and for how long, then `Connected as <name>. Token saved to <path>.`; `runner` goes straight on to polling and `runner login` adds the command to run next. A printed command is always `npx hawkeye-review …`, the way the README runs it. Errors carry no `error:` prefix, the exit code says that: what went wrong, that nothing was lost when that is true, and the next step, each its own sentence. A failed review starts with `Review failed.` in red and ends with where the run is kept. Paths under the home directory print with `~`.
...
Roots and shell: `hk-root`, `hk-topbar[data-sticky]`, `hk-footer`, `hk-wordmark`, `hk-wordmark-text`, `hk-nav`, `hk-topbar-end`, `hk-account-name`, `hk-account-initial`, `hk-fresh`, `hk-menu` (a `<details>`), `hk-menu-panel`, `hk-menu-who`, `hk-menu-theme`, `hk-footer-theme`, `hk-menu-row`, `hk-menu-legal`, `hk-progress`, `hk-page`.
