...
**Today.** The claim endpoint long-polls inside a serverless function: up to 25 s per call, a queue check every 5 s, and the stale-claim sweep once per claim call. That is the shape this section replaces. Everything below is the target. Two of its rules are unmet today: the sweep runs inside a request, and an idle runner claims again after a fixed second with no back-off. The indexed-predicate rule is met: `job_claimable` (on `not_before` where `state = 'queued'`) serves the claim and `job_stale` (on `heartbeat_at` where `state = 'claimed'`) serves the sweep, both partial so finished jobs stay out of them.
