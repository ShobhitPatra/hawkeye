...
Sign-in uses the GitHub App's OAuth client: in the App settings set the callback URL to `<BETTER_AUTH_URL>/api/auth/callback/github`, generate a client secret, and grant the account permission Email addresses: read (Better Auth needs an email; GitHub hides private ones otherwise). Env (see `apps/web/.env.example`): `DATABASE_URL`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET`; later: `GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY`, `GITHUB_WEBHOOK_SECRET`.
Hosted: Vercel project rooted at `apps/web` with a Neon Postgres and the same env. Run `pnpm --filter web db:migrate` against Neon before the first deploy.
