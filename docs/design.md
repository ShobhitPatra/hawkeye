...
**Today.** The claim endpoint long-polls inside a serverless function: up to 25 s per call with a queue check every 5 s. The long-poll is the shape this section replaces; everything below is the target. Its three cheap rules are met. The fleet-wide stale-claim sweep runs on a schedule, not in a request: `/api/internal/sweep`, guarded by `CRON_SECRET`, called every five minutes by a GitHub Actions workflow for the hosted instance (Vercel's Hobby cron fires once a day and stays as a backstop). A claim also puts back the asking user's own stale claims, once per call, since only that user's runners can take them; a runner that restarts after dying mid-review gets its job back five minutes after the last heartbeat, whatever the schedule does. The hot predicates are indexed: `job_claimable` (on `not_before` where `state = 'queued'`) serves the claim and `job_stale` (on `heartbeat_at` where `state = 'claimed'`) serves the sweep, both partial so finished jobs stay out of them; `job_by_armed_pr` (on `armed_pr_id, created_at, id`, all states) serves the one question that spans finished jobs, whether a newer head has come along, which the heartbeat asks every 30 s and the sweep asks of each stale claim. An idle runner backs off: one whose user has no pull request with reviews on is answered at once, with no hold and no queue check, and told to ask again in 60 seconds.
...
- **No fleet-wide housekeeping in a request.** Sweeps across the fleet and other fleet-wide maintenance run on a schedule, never inside a request a client makes. One runner must never do the whole fleet's work. A request may tidy its own user's rows when that is the moment they matter, as the claim does with the user's stale claims, and only through an index that keeps the check to the rows in question.
...
**`/prs`** lists the open pull requests you authored across your installations, one row each: the title with repo and number, one status word about the review (Reviews on while nothing has run yet, Queued, Waiting with the runner offline, In review, Run failed, the last verdict, or Off when reviews are paused), the round count, open findings and when the last review ended. Nothing in a row acts; every title opens the pull request's page, which exists for every listed pull request, armed or not.

**Per-pull-request pages** list the PR's runs with status, verdict (and the model's own verdict when it disagreed), turns, duration and the posted review link, and its findings with their stable ids, the head each was first seen on and whether it was resolved. When the latest run reviewed but its review went out in the short form or did not reach GitHub, a state sentence above the verdict says so, with GitHub's reason, so the dashboard explains a short or missing comment. The title row holds the page's actions: Pause reviews or Turn reviews on (turning on queues a review for the current head and marks the PR for review on every later push; the server refuses a pull request the signed-in user did not author), and, while reviews are on and a review has been posted, Review from scratch, which queues a job that reviews the full merge-base diff with no prior findings and no interdiff (`job.from_scratch`); its round replaces the living review in place as round 1 of a new history, later rounds count from it, the rounds before it drop out of view, and their inline threads are left as they are.

**Freshness.** Every dashboard page refetches every 30 s while visible and when the tab regains focus; the top bar says in small text how long ago it was updated, and the progress line shows under it while a fetch runs. The part of the list that comes from GitHub (which pull requests are open, their titles) is held for a minute per user — 15 s when an installation refused — so it can be up to a minute behind while review states are 30 s fresh. Titles on Overview and the detail page come from the armed row, stored when the pull request is armed and refreshed on `edited` deliveries and every push; a row armed before titles were stored is filled from GitHub on its first view.
...
2. **Public.** Repository public under MIT, the hosted instance at hawkeye.reviews, a README for newcomers, not-found and error pages, the sign-up funnel. **(shipped)**
...
- **The job carries the user's settings:** turn and wall-clock limits, the prompt override, the harness and model (chosen together on Settings: `claude-code` or `codex`, and one of that CLI's pinned models, else the CLI's default; a model that has left the list keeps working for whoever saved it, and a saved model belonging to the other CLI is not sent; a daemon started with `--model` keeps its own for Claude Code; a new model is added to the list and shipped; when the claude CLI refuses the chosen model, the runner reviews once more on its default in the same run, and the run row and the review footer say which model was refused, while a Codex review on a model the plan does not allow fails with Codex's own error; a runner older than the release that ships the refusal fallback fails the run as before and reports no refused model) and the concurrency (1 to 3, default 1).
