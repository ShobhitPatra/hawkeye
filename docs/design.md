...
**Flags.** `--sandbox docker|podman` runs each review's CLI inside a container that mounts only the runs directory and the CLI's login files at their host paths (so the harness needs no path mapping), with the user's uid, `HOME` set to the host home and the host network; the image is built once from an embedded Dockerfile (Node 22, git, the Claude Code and Codex CLIs) and the daemon logs `sandbox` while it builds and when it is ready. `--contract <path>` overrides the built-in contract (else the job's per-user override, else the default); `--model <name>` picks the claude CLI model for every job this runner claims.
