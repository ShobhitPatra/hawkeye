...
- **Anything else with a timer.** The repository's `.github/workflows/sweep.yml` calls the route every five minutes for the hosted instance; in a fork, set the repository secret `CRON_SECRET` and the variable `CONTROL_PLANE_URL` and drop the repository check at the top of the job. A cron line on any machine does the same:
