...
- Draft PRs can be armed; reviews start when ready unless the user opts in to draft reviews. A draft opened by a linked user is left alone until `ready_for_review`, which turns it on then, unless the user reviews drafts, in which case it turns on at open. Because the ready event carries no opening date, a draft that was already open when this shipped also turns on when it becomes ready; that is the one case where an older pull request is armed.
