...
Confirming an action is a state word changing, with at most a 160ms color transition. Content that replaces other content, on a refresh or after a click, fades in over 200ms with a 3px rise (`.hk-arrive`); nothing exits with motion. A pressed button scales to 0.98 for 120ms; hover styles apply only to a fine pointer. Reduced motion turns everything static. No pulsing dots, no spinners, no scroll reveals, no animated numbers.
